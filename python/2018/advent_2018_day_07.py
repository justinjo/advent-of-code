...
def get_step_order(input_arr: list[str]) -> defaultdict[str, list[str]]:
...
def get_blocking_order(input_arr: list[str]) -> defaultdict[str, list[str]]:
    step_order = defaultdict(list)
    for s in input_arr:
        words = s[5:-11].split(" ")
        blocking_step = words[0]
...
def get_first_steps(step_order: defaultdict[str, list[str]]) -> list[str]:
    first_steps = []
...
def part_one(input_arr: list[str]) -> str:
    step_order = get_step_order(input_arr)
    blocking_order = get_blocking_order(input_arr)
    first_steps = get_first_steps(step_order)
    return get_order(step_order, blocking_order, first_steps)
...
def part_two(input_arr: list[str]) -> int:
    ...
    blocking_order = get_blocking_order(input_arr)
    first_steps = get_first_steps(step_order)
    return construct(step_order, blocking_order, first_steps)


input_arr: list[str] = open("advent_2018_day_07.txt").read().splitlines()
