...
def match_regex(string: str) -> Coord3D:
...
def get_moon_coords(input_arr: list[str]) -> MoonsMap:
    moons = {}
    for i in range(len(input_arr)):
        moons[MOONS[i]] = {
            "pos": match_regex(input_arr[i]),
            "vel": DEFAULT_COORD,
            "acc": [0, 0, 0],
        }
    return moons
...
def calculate_acceleration(moon_1: Moon, moon_2: Moon) -> Coord3D:
    a_x = a_y = a_z = 0
...
def simulate_motion(moons: MoonsMap, steps: int = 1) -> None:
    for _ in range(steps):
        for i in range(len(moons)):
            moon_i = moons[MOONS[i]]
            for j in range(i + 1, len(moons)):
                moon_j = moons[MOONS[j]]
                a_x, a_y, a_z = calculate_acceleration(moon_i, moon_j)
                moon_i["acc"][0] += a_x
                moon_i["acc"][1] += a_y
                moon_i["acc"][2] += a_z
                moon_j["acc"][0] -= a_x
...
def calculate_total_energy(moons: MoonsMap) -> int:
    total_energy = 0
    for moon in moons.values():
...
def get_state(moons: MoonsMap, vertex_i: int) -> CoordState:
...
def find_steps_in_loop(moons: MoonsMap) -> tuple[int, int, int]:
    ...
    x_state = get_state(moons, 0)
    y_state = get_state(moons, 1)
    z_state = get_state(moons, 2)
...
def print_moons(moons: MoonsMap, step: str = "") -> None:
    if step:
...
def part_one(input_arr: list[str]) -> int:
    moons = get_moon_coords(input_arr)
...
def part_two(input_arr: list[str]) -> int:
    moons = get_moon_coords(input_arr)
    x, y, z = find_steps_in_loop(moons)
    return math.lcm(x, y, z)


input_arr: list[str] = open("advent_2019_day_12.txt").read().splitlines()
